...
const parseZodObjectDef = (def, refs) => {
    ...
    for (const propertyName of Object.keys(shape)) {
        const node = zodSelectorFunction((_a = shape[propertyName]) === null || _a === void 0 ? void 0 : _a._def, Object.assign(Object.assign({}, refs), { path: refs.path.concat([propertyName]) }));
...
function nodeAndInputSchemaFromInputs(inputs, _routerPath, options, addDataFunctions) {
    ...
    if (inputs.length > 1) {
        const allInputsAreZodObjects = inputs.every((input) => input instanceof zod.z.ZodObject);
